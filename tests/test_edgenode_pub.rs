...
async fn normal_publish() {
    // Setup container
    let absolute_conf = std::fs::canonicalize(std::path::Path::new("tests/anon.conf"))
        .expect("Error creating absolute path for Mosquitto config file");
    ...
    let container = GenericImage::new("eclipse-mosquitto", &ver)
        ...
        .with_wait_for(WaitFor::message_on_stdout(format!(
            "mosquitto version {ver} running"
        )))
        .with_mount(Mount::bind_mount(
            absolute_conf
                .to_str()
                .expect("Error getting str from absolute path"),
    ...
    // Setup subscriber
    let mut paho_cli =
        paho_mqtt::Client::new(host.as_str()).expect("Error creating Paho subscriber");
    ...
    paho_cli
        .connect(None)
        .expect("Error connecting Paho subscriber");
    paho_cli
        .subscribe(&topic, 0)
        .expect("Error subscribing Paho subscriber");
    ...
    spb_edgenode_cli
        .connect(None)
        .expect("Error connecting Edge Node");
    spb_edgenode_cli
        .publish(topic.to_string(), payload.to_string())
        .expect("Error publishing from Edge Node");
    ...
    spb_edgenode_cli
        .disconnect(None)
        .expect("Error disconnecting Edge Node");

    if paho_cli.is_connected() {
        paho_cli
            .disconnect(None)
            .expect("Error disconnecting Paho subscriber");
    }
}
